...
플러그인 창의 **현재 Job** 섹션에는  
서버에서 처리 중인 학습 작업에 대해 다음 항목들이 표시된다.

- **Job ID**  
  서버에서 각 리타게팅 모델 요청마다 부여하는 **고유 식별자**이다.  
  다운로드된 결과 폴더 이름에도 이 Job ID가 사용된다.  
  예: `Assets/DistilledModels/<JobID>/...`

- **File**  
  리타게팅 모델 요청에 사용된 FBX 파일의 **파일명**이다.  
  어떤 입력 파일로 Job이 생성되었는지 확인할 때 사용한다.

- **Status**  
  현재 Job의 **처리 상태 문자열**을 서버에서 전달받아 그대로 표시한다.  
  예를 들어 다음과 같은 값이 사용될 수 있다.
  - `queued` : 서버에서 작업 요청이 접수되었으며 대기 중인 상태
  - `running` : 서버에서 작업이 진행 중인 상태
  - `done` : 서버에서 학습 작업이 정상적으로 완료된 상태
  - `error` : 상태 조회나 처리 중 오류가 발생한 상태
  - `downloaded` : Unity 플러그인에서 결과를 성공적으로 다운로드한 상태

- **Message**  
  서버에서 전달하는 **상세 메시지**로,  
  현재 진행 상황이나 오류 원인 등이 담긴다.  
  문제가 발생했을 때는 이 Message를 우선 확인하는 것이 좋다.

- **R2ET 남은 시간**  
  Teacher 모델(R²ET) 관련 처리 단계에 대해  
  서버가 추정한 **남은 시간(초 단위)**을 표시한다.  
  플러그인 UI에서는 이를 `분:초` 형식(예: `03:25`)으로 보여주며,  
  완료되었거나 남은 시간이 없을 경우 **완료**로 표시된다.

- **Student 남은 시간**  
  distilR²ET Student 모델 학습 단계에 대해  
  서버가 추정한 **남은 시간(초 단위)**을 표시한다.  
  마찬가지로 UI에서는 `분:초` 형식으로 표시되며,  
  완료 시 **완료**로 표시된다.
